...
records = repmat(localRecord(),2*numel(manifest.sources)+1,1);
...
for index = 1:numel(manifest.sources)
    ...
    records(cursor) = localRecord( ...
        string(source.acceptedMatlabPackage.path), ...
        string(source.acceptedMatlabPackage.sha256), ...
        "v17a_source_package:"+string(source.sourceId));
    cursor = cursor+1;
    records(cursor) = localRecord(string(source.certification.path), ...
        string(source.certification.sha256), ...
        "v17a_source_certificate:"+string(source.sourceId));
end
end
