...
%======================================================================
    ...
    methods
        ...
        %--------------------------------------------------------------
        function [z,fval,exitflag,output,lambda] = solve(obj,costFun,z0,lb,ub,nonlFun)
        %SOLVE Run SQP.
        %
        % Inputs:
        ...
        % Outputs match fmincon-style enough for nMPC diagnostics.
            ...
            for iter = 1:opts.MaxIterations
                ...
                % 2. Merit line search
                % -----------------------------------------------------
                phi0 = obj.merit(costFun,nonlFun,z,lb,ub,opts.MeritPenalty);
                ...
                for ls = 1:opts.MaxLineSearch
                    ...
                    phiTrial = obj.merit(costFun,nonlFun,zTrial,lb,ub,opts.MeritPenalty);

                    meritTol = opts.MeritAcceptTolerance * max(1,abs(phi0));
                    
                    if phiTrial <= phi0 - opts.LineSearchC1*alpha*predictedDecrease + meritTol || ...
                       phiTrial <= phi0 + meritTol
                        accepted = true;
                        break
                    end

                    alpha = opts.LineSearchBeta * alpha;
                ...
                % -----------------------------------------------------
                zNew = z + alpha*d;
                zNew = min(max(zNew,lb),ub);

                try
                    [fNew,gNew,cNew,ceqNew,gradcNew,gradceqNew] = ...
    ...
    methods(Access=private)
        ...
        %--------------------------------------------------------------
        function [f,g,c,ceq,gradc,gradceq] = evalAll(~,costFun,nonlFun,z)
            [f,g] = costFun(z);
