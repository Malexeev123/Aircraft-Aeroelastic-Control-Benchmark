...
p.addParameter('make_compatible',false,@islogical);
...
pointsCell = cell(numel(setupDirs),1);
for i = 1:numel(setupDirs)
    sd = char(setupDirs{i});
    [~,fm] = AeroFlex.sched.resolveSetupDir(setupDirs{i});
    ...
    pointsCell{i} = AeroFlex.sched.extractPoint( ...
        cfg,Sbem.beam,Saer.aero,Sbas.base,trim, ...
        'source_dir',sd,'body_case',body_case);
end
points = vertcat(pointsCell{:});
...
ROMlib.created = char(datetime("now","Format","yyyy-MM-dd HH:mm:ss"));
ROMlib.version = 1;
...
if opt.make_compatible
    ROMlib = AeroFlex.sched.finalizeCompatibleLibrary(ROMlib, ...
        'ref_id', opt.ref_id);
end
ROMlib.compatibleCoordinates = logical(opt.make_compatible);
